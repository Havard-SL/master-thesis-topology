...
\begin{remark} \label{rem:big_iso}
    ...
    \begin{center}
        \begin{tikzpicture}
            ...
            \draw[math]
                (m-1-1) edge node[marked, below] {\sim} node {g_1} (m-2-1)

                (m-2-1) edge node[marked, below] {\sim} node {g_2} (m-3-1)

                (m-3-1) edge node[marked, below] {\sim} node {g_3} (m-4-1);
        \end{tikzpicture}
    \end{center}
...
\begin{remark} \label{rem:F_properties}
        ...
        \begin{tikzpicture}
            ...
            \draw[math]
                (m-1-1) edge node {f} (m-1-2)
                    edge node {\psi_A} node[marked, below] {\sim} (m-2-1)
                (m-1-2) edge node {\psi_B} node[marked, below] {\sim} (m-2-2)
        ...
        \begin{tikzpicture}
            ...
            \draw[math]
                (m-1-1) edge node {f_*} (m-1-2)
                    edge node {(\psi_A)_*} node[marked, below] {\sim} (m-2-1)
                (m-1-2) edge node {(\psi_B)_*} node[marked, below] {\sim} (m-2-2)
    ...
    \begin{center}
        \begin{tikzpicture}[every node/.style={scale=0.93}] \label{tikz:f_star}
            ...
            \draw[math]
                (m-1-1) edge node {\phi_A} node[marked, below] {\sim} (m-1-2)
                    edge node {F(f)_*} (m-2-1)
                (m-1-2) edge node {\phi_B \circ F(f)_* \circ \phi_A^{-1}} (m-2-2)
                    
                (m-2-1) edge node {\phi_B} node[marked, below] {\sim} (m-2-2);
        \end{tikzpicture}
    \end{center}
