...
This thesis will refrain from proving that \autoref{def:H_bullet_dg_category} is a DG-category, since a lot of the calculations involved in showing that \( H^\bullet(\Cc) \) satisfies \cite[Diagram 6.9, Diagram 6.10]{Borceux_1994} is similar to the proof of \autoref{lem:dg-composition_associative}, and would also require defining the associativity morphism (properly), as well as the left and right tensor unit. Defining the latter morphisms is straight-forward, but would require a lot of boilerplate for something that, in the end, is not necessary for the goal of this thesis.
...
\begin{definition}
    ...
    \[
        ...
        \{
            ...
            \mid \\
            d_{\Cc, |f_3| + |f_2| - 1}(s) = \bar{f_3} \circ f_2, \,
            d_{\Cc, |f_2| + |f_1| - 1}(t) = \bar{f_2} \circ f_1
        \}.
    \]
...
\begin{remark}
    ...
    Look at the following equation (with the degrees of the differentials omitted for readability)
    \begin{align*}
        ...
        &= d_{\Cc(X_1, X_4)}(c_{\Cc, |g_1| + |g_2| + |g_3| - 1}(\bar{s} \otimes g_1)) \\
        &\hspace{0.4cm} + d_{\Cc(X_1, X_4)}(c_{\Cc, |g_1| + |g_2| + |g_3| - 1}(\bar{g_3} \otimes t)) \\
        \intertext{by the fact that composition is a chain morphism, it follows that}
        &= c_{\Cc, |g_1| + |g_2| + |g_3|}(d_{\Cc(X_2, X_4) \otimes \Cc(X_1, X_2)}(\bar{s} \otimes g_1)) \\
        &\hspace{0.4cm} + c_{\Cc, |g_1| + |g_2| + |g_3|}(d_{\Cc(X_3, X_4) \otimes \Cc(X_1, X_3)}(\bar{g}_3 \otimes t))
        \intertext{by the definition of the differential of the tensor products, it follows that}
        &= c_{\Cc, |g_1| + |g_2| + |g_3|}((-1)^{|g_2| + |g_3|}(\bar{g}_3 \circ g_2) \otimes g_1) \\
        &\hspace{0.4cm} + c_{\Cc, |g_1| + |g_2| + |g_3|}((-1)^{|g_3|}\bar{g}_3 \otimes (\bar{g}_2 \circ g_1)) \\
        \intertext{by pulling out every sign, it follows that}
        &= (-1)^{|g_2| + 2|g_3| - 1}c_{\Cc, |g_1| + |g_2| + |g_3|}((g_3 \circ g_2) \otimes g_1) \\
        &\hspace{0.4cm} + (-1)^{|g_2| + 2|g_3| - 2}c_{\Cc, |g_1| + |g_2| + |g_3|}(g_3 \otimes (g_2 \circ g_1)) \\
        \intertext{by the definition of composition of morphisms in a dg-diagram, it follows that}
