...
\begin{notation}
    ...
    \[
        g \circ f := c_{|g| + |f|}^{\Cc}(g \otimes f) \in \Cc\tuple*{s(f), t(g)}_{|g| + |f|},
    \]
    where
    \[
        c_{\Cc} := \set*{c_i^{\Cc}}_{i \in \Zb}, \quad \text{with} \quad c_i^{\Cc}: \tuple*{ \Cc\tuple*{s(g), t(g)} \otimes \Cc\tuple*{s(f), t(f)} }_i \to \Cc\tuple*{s(f), t(g)}_i,
    \]
...
\end{lemma}
\begin{proof}
    By definition, composition in a DG-diagram is the composition of the \( R \)-bilinear morphism that takes \( f \times g \mapsto f \otimes g \), as well as the \( \Mod(R) \) morphisms, \( \iota_{i, j} \) and \( c_{i + j}^{\Cc} \).
\end{proof}
...
\begin{remark}
    ...
    \end{align*}
    Assuming these are well-defined \( R \)-bilinear morphisms, denote by \( \alpha_n \) the unique morphism they define by \autoref{lem:map_out_of_tensor_unique}, and the entire chain morphism by \( \alpha := \set*{\alpha_i}_{i \in \Zb} \). \footnote{
...
\begin{remark}
    ...
    \begin{enumerate}
        \item {
            ...
            \[
                g_i \otimes f_j \in \ker(d_n^{\Cc(B, C) \otimes \Cc(A, B)}).
            \]
            ...
            \[
                d_n^{\Cc(B, C) \otimes \Cc(A, B)}(g_i \otimes f_j) = 0.
            \]
        }
        \item {
            We need to show that the values of the \( \alpha_{i, j} \)'s are independent of the choice of representatives.
            ...
            As an example, consider the case above of \( g_i \otimes b_f \). Since \( b_f \) is a boundary in \( \Cc(A, B)_j \), there is some \( b_f' \in \Cc(A, B)_{j - 1} \) such that \( d_{j - 1}^{\Cc(A, B)}(b_f') = b_f \). Thus,
            \begin{align*}
                d_{i + j - 1}^\Cc(B, C) \otimes \Cc(A, B)(&(-1)^i g_i \otimes b_f') \\
                &= (-1)^i d_i^{\Cc(B, C)}(g_i) \otimes b_f' + (-1)^i (-1)^i g_i \otimes d_{j - 1}^{\Cc(A, B)}(b_f') \\
...
This thesis will refrain from proving that the above definition of \( H^\bullet(\Cc) \) and any future alleged DG-category actually satisfy the associativity axiom and the unit axiom as defined by \cite[Definition 6.2.1]{Borceux_1994}. The proofs of the two axioms are often straightforward, and is more of a technical detail than a necessity for our use case. The proof of these two axioms would almost certainly rely on some version of \autoref{lem:map_out_of_tensor_unique}.
...
\begin{definition}
    ...
    \begin{multline*}
        ...
        \{
            ...
            \mid [g_1] = [f_1], \quad [g_2] = [f_2], \quad [g_3] = [f_3], \\
...
\begin{remark}
    ...
    We want to show that \( \bar{s} \circ g_1 + \bar{g_3} \circ t \) is a cycle in \( \Cc(X_1, X_4)_{|g_1| + |g_2| + |g_3| - 1} \).

    Let \( n := |g_1| + |g_2| + |g_3| \). Consider the following equation
    \begin{align*}
        &d_{n - 1}^{\Cc(X_1, X_4)}(\bar{s} \circ g_1 + \bar{g_3} \circ t) \\
        &= d_{n - 1}^{\Cc(X_1, X_4)}(\bar{s} \circ g_1) + d_{n - 1}^{\Cc(X_1, X_4)}(\bar{g_3} \circ t) \\
        \intertext{by the definition of composition of morphisms in a DG-diagram,}
        &= d_{n - 1}^{\Cc(X_1, X_4)} \circ c_{n - 1}^{\Cc}(\bar{s} \otimes g_1) + d_{n - 1}^{\Cc(X_1, X_4)} \circ c_{n - 1}^{\Cc}(\bar{g_3} \otimes t) \\
        \intertext{by composition being a chain morphism,}
        &= c_n^{\Cc} \circ d_{n - 1}^{\Cc(X_2, X_4) \otimes \Cc(X_1, X_2)}(\bar{s} \otimes g_1) + c_n^{\Cc} \circ d_{n - 1}^{\Cc(X_3, X_4) \otimes \Cc(X_1, X_3)}(\bar{g}_3 \otimes t)
        \intertext{by the definition of the differential of the tensor products,}
        &= c_n^{\Cc}((-1)^{|g_2| + |g_3|}(\bar{g}_3 \circ g_2) \otimes g_1) + c_n^{\Cc}((-1)^{|g_3|}\bar{g}_3 \otimes (\bar{g}_2 \circ g_1)) \\
        \intertext{pulling out every sign,}
        &= (-1)^{|g_2| + 2|g_3| - 1}c_n^{\Cc}((g_3 \circ g_2) \otimes g_1) + (-1)^{|g_2| + 2|g_3| - 2}c_n^{\Cc}(g_3 \otimes (g_2 \circ g_1)) \\
        \intertext{by the definition of composition of morphisms in a DG-diagram,}
