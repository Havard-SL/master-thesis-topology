...
If at the end of the final example one wasn't so lucky and could choose a \( \phi \), such that every \( \psi \) could be expressed, the calculations would have become much more tedious. Then one would have to find every possible \( \phi \) and from every possible \( \phi \) then find every possible \( \psi \). Combining this together with the fact that working with morphisms in \( \Mc \) is not simple, and one could end up with a very complicated and annoying calculation. Combining this with the fact that simply calculating an appropriate distinguished triangle was tedious, and the calculations become even worse.
