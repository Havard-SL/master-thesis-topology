...
\begin{example}% Is it allowed to have a proof to an example?
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \item {
            For any object \( P \in \Tc \) with \( P \neq 0 \), one has that \( f = \Id_P \in \Tc(P, P) \), but \( (\Id_P)_*: \Tc(P, P) \to \Tc(P, P) \) is \emph{not} the zero map, since \( \Id_P \circ \Id_P = \Id_P \neq 0 \). So \( \Pc \subseteq \set{0} \).

            If \( P = 0 \), then for any \( f \in \Tc(X, Y) \), one has that \( \Tc(0, Y) \) is the zero map, so the statement holds. 
        }
        \item {
