...
\begin{definition}[Adams resolution w.r.t. an injective class] \label{def:adams_resolution_injective_class} % Need stable? TODO
    ...
    Where every \( i_n \in \Nc \), and \( I_n \in \Ic \), and circled arrows denote degree shifting maps, with \( \delta_n \in \Tc(I_n, \Sigma(Y_{n + 1})) \). % More descriptive of what degree shifting maps are.
...
\begin{remark} % Is Y_s for s < 0 defined? Other notation for sum of maps, since i_0 doesn't go from Y_0. TODO
    Let \( (\Ic, \Nc) \) be an injective class in \( \Tc \).
    ...
    \begin{center}
        \begin{tikzpicture}
            ...
            \draw[math]
                (m-1-1) edge node {\directsum_{s \in \Nb_0, t \in \Zb} (i_s)_*} (m-1-3)
                (m-1-3) edge node {\directsum_{s \in \Nb_0, t \in \Zb} (p_s)_*} (m-2-2)

                (m-2-2) edge node {\directsum_{s \in \Nb_0, t \in \Zb} (\delta_s)_*} (m-1-1);
        \end{tikzpicture}
    \end{center}
\end{remark}
