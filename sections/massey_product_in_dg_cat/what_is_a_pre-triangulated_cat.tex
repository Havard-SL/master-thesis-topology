...
\begin{remark}
    ...
    By looking at
    \[
        \sigma_A \circ \sigma_A^{-1}: \Sigma(A) \to \Sigma(A)
    \]
    one can see that it is exactly the identity morphism, and likewise for \( \sigma_A^{-1} \circ \sigma_A \), and the morphism is therefore the inverse of \( \sigma_A \).

    Furthermore, for \( f \in \C_{\dg}(\Mod(R))(A, B) \), homogeneous of degree \( i \), consider the morphism
    \[
        \sigma_B \circ f \circ \sigma_A^{-1}.
    \]
    Looking at what the morphism is doing element-wise
    \[
        \begin{pmatrix}
            \vdots \\
            a_1 \\
