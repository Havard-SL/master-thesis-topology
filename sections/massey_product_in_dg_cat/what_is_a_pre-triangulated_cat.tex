...
\begin{remark}[Functor category structure from Borceux]
    ...
    \begin{center}
        ...
        \begin{tikzpicture}
            ...
            \draw[->, math] ($(m-1-1.south) + (0.25, 0)$) edge node {\prod_{A', A'' \in \Cc} \phi_{A', A''}} ($(m-2-1.north) + (0.25, 0)$);
            \draw[->, math] ($(m-1-1.south) + (-0.25, 0)$) edge node[swap] {\prod_{A', A'' \in \Cc} \widetilde{\phi}_{A', A''}} ($(m-2-1.north) + (-0.25, 0)$);
        \end{tikzpicture}
    \end{center} 
\end{remark}
