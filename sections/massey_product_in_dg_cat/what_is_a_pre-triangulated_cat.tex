...
\begin{definition}[\( \C_{\dg}(\Mod(R)) \)]
    ...
    \begin{enumerate}
        ...
        \item {
            Let \( A, B \in \C_{\dg}(\Mod(R)) \). Then let \( \class{A, B} \) denote the internal hom (\autoref{def:internal_hom_of_chain_complexes_over_Mod(R)}) of \( \C(\Mod(R)) \) with respect to \( A, B \) as objects from \( \C(\Mod(R)) \).

            Then let \( \C_{\dg}(\Mod(R))(A, B) := \class{A, B} \).
            % \[ 
            %     \tuple{\C_{\dg}(\Mod(R))(A, B)}_i := \coprod_{j \in \Zb} \Mod(R)(A_j, B_{j + i}).
            % \]
            % Let \( d_A \) and \( d_B \) be the differential of \( A \) and \( B \) as objects of \( \C(\Mod(R)) \), respectively.

            % Then define a chain complex \( \C_{\dg}(\Mod(R))(A, B) \), where the \( i \)-th component is \( \tuple{\C_{\dg}(\Mod(R))(A, B)}_i \) and with the differentials defined as follows
            % \begin{align*}
