...
% TODO: Need to show that the structure of coproduct makes it possible to define differential this way.
\begin{definition}
    ...
    \begin{center}
        \begin{tikzpicture}
            \diagram{m}{1cm}{1cm} {
                A \otimes B: \& \cdots \& (A \otimes B)_{-1} \& (A \otimes B)_0 \& (A \otimes B)_1 \& \cdots \\
            };
        ...
        \end{tikzpicture}
    \end{center}
    % Here a \otimes b \in (A \otimes B)_{i + j} is valid notation because elements in the coproduct is often depicted as a sum of the two elements.
...
\begin{definition}[Internal hom of chain complexes]
    \label{def:internal_hom_of_chain_complexes}
    Let \( A, B \in \C(\Cc) \) and let \( d_A \) and \( d_B \) be the differential of \( A \) and \( B \) respectively.

    Then define a chain complex \( \class{ A, B } \), where the \( i \)-th component is \( \coprod_{j \in \Zb} \Cc(A_j, B_{j + i}) \) and with the differentials defined as follows
    \begin{align*}
        d_i: \coprod_{j \in \Zb} \Cc(A_j, B_{j + i}) &\to \coprod_{j \in \Zb} \Cc(A_j, B_{j + i + 1}) \\
