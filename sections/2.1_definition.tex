...
There are multiple different, but equivalent, definitions of a triangulated category with different axioms. This thesis will use the following definition.

\begin{definition}[Triangulated category]
    \label{def:triangulated_category}
    Let \( \Tc \) be an additive category, and let \( \Sigma: \Tc \to \Tc \) be an additive autoequivalence. In addition, let \( \Delta \) be a class of triangles (\autoref{def:triangles}) in \( \Tc \).
    ...
    \begin{enumerate}[label={(\bfseries T\arabic*)}]
        \item {
            ...
            \begin{enumerate}
                \item {
                    For any \( f: X \to Y \) in \( \Tc \), then there is some object \( C_f \in \Tc \), called a \emph{cone of \( f \)}, such that there is a triangle
                    \begin{center}
                        \begin{tikzpicture}
                            \diagram{m}{1cm}{1cm} {
                                X \& Y \& C_f \& \Sigma X \\
                ...
                \item {
                    For any \( X \in \Tc \), then the \emph{trivial triangle of \( X \)}, which is the following triangle
                    \begin{center}
                        \begin{tikzpicture}
                            \diagram{m}{1cm}{1cm} {
                                X \& X \& 0 \& \Sigma X \\
        ...
        \item {
            ...
            \begin{center}
                ...
                \end{tikzpicture}
            \end{center}
            in \( \Delta \), then both the \emph{shifted triangles}
            \begin{center}
                \begin{tikzpicture}
                    \diagram{m}{1cm}{1cm} {
                        B \& C \& \Sigma A \& \Sigma B, \\
