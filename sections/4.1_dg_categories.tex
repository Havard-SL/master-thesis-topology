...
First, it can be helpful to define some notation.
\begin{notation}
    Let \( R \) be a commutative ring with identity in this section.

    Let \( d_{A, n} \) denote the \( n \)-th differential of a chain complex \( A \).
\end{notation}

The first definition necessary to understanding DG-categories, is the tensor product of chain complexes.

\begin{definition}[Tensor product of \( \C(\Mod(R)) \)]
    ...
    This is called the \emph{tensor product of \( \C(\Mod(R)) \)}.
\end{definition}
...
\begin{remark}[Symmetry of tensor product in \( \C(\Mod(R)) \)]
    For two \( A, B \in \C(\Mod(R)) \), there exist an isomorphism
    \[
    ...
    By \autoref{lem:map_out_of_tensor_unique} every \( s_n \) is well-defined. It is also clear that every \( s_n \) is an isomorphism. It only remains to verify that \( s = \tuple{s_n}_{n \in \Zb} \) is a chain morphism.
    ...
    \begin{align*}
        ...
        &= (-1)^{(i + 1)j} b \otimes d_{A, i}(a) + (-1)^{i + i(j + 1)} d_{B, j}(b) \otimes a - (-1)^{ij} d_{B, i}(b) \otimes a - (-1)^{j + ij} b \otimes d_{A, j}(a) \\
        &= WIP
    \end{align*}
\end{remark}

Another definition closely linked to the tensor product in \( \C(\Mod(R)) \) is the internal hom.

\begin{definition}[Internal hom of \( \C(\Mod(R)) \)]
