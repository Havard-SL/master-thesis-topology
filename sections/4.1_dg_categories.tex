        ...
        \item {
            ...
            \begin{center}
                ...
                \begin{tikzpicture}
                    ...
                    \draw[math]
                        (m-1-1) edge (m-1-2)
                        (m-1-2) edge node {d_{-1}^{A \otimes B}} (m-1-3)
                        (m-1-3) edge node {d_0^{A \otimes B}} (m-1-4)
                        (m-1-4) edge (m-1-5);
                \end{tikzpicture}
            \end{center}
            ...
            \begin{center}
                \begin{tikzpicture}
                    ...
                    \draw[math]
                        ...
                        (m-1-2) edge (m-1-3)
                        (m-1-3) edge node {d_{-1}^{A \otimes B}} (m-1-4)
                            edge node {\coprod\limits_{i + j = -1} f_i \otimes g_j} (m-2-3)
                        (m-1-4) edge node {d_0^{A \otimes B}} (m-1-5)
                            edge node {\coprod\limits_{i + j = 0} f_i \otimes g_j} (m-2-4)
                        (m-1-5) edge (m-1-6)
                            edge node {\coprod\limits_{i + j = 1} f_i \otimes g_j} (m-2-5)

                        (m-2-2) edge (m-2-3)
                        (m-2-3) edge node {d_{-1}^{A' \otimes B'}} (m-2-4)
                        (m-2-4) edge node {d_0^{A' \otimes B'}} (m-2-5)
                        (m-2-5) edge (m-2-6);
                \end{tikzpicture}
            \end{center}
        }
...
In order to see that the differential on \( A \otimes B \) is well-defined, we need the following lemma, which will be useful later as well.
...
Given some \( f \in \Mod(R)\tuple*{ (A \otimes B)_n, C } \) we have that for any \( i + j = n \), the map \( g_{i, j}(a, b) := f(a \otimes b) \) is an \( R \)-bilinear morphism, and therefore \( \set{g_{i, j}}_{i + j = n} \) uniquely define \( f \). In a way, the \( g_{i, j} \)'s ``represent'' \( f \).
        ...
        \item {
            ...
            \begin{center}
                \begin{tikzpicture}
                    ...
                    \draw[math]
                        (m-1-2) edge (m-1-3)
                        (m-1-3) edge node {d_{-1}^{[A, B]}} (m-1-4)
                        (m-1-4) edge node {d_0^{[A, B]}} (m-1-5)
                        (m-1-5) edge (m-1-6);
                \end{tikzpicture}
            \end{center}
        ...
        \item {
            ...
            \begin{center}
                \begin{tikzpicture}
                    ...
                    \draw[math]
                        (m-1-1) edge node {[f, g]} (m-2-1)
                        (m-1-2) edge (m-1-3)
                        (m-1-3) edge node {d_{-1}^{[A, B]}} (m-1-4)
                            edge node {\prod\limits_{i \in \Zb} (g_{i - 1})_* \circ (f_i)^*} (m-2-3)
                        (m-1-4) edge node {d_0^{[A, B]}} (m-1-5)
                            edge node {\prod\limits_{i \in \Zb} (g_i)_* \circ (f_i)^*} (m-2-4)
                        (m-1-5) edge (m-1-6)
                            edge node {\prod\limits_{i \in \Zb} (g_{i + 1})_* \circ (f_i)^*} (m-2-5)

                        (m-2-2) edge (m-2-3)
                        (m-2-3) edge node {d_{-1}^{[A', B']}} (m-2-4)
                        (m-2-4) edge node {d_0^{[A', B']}} (m-2-5)
                        (m-2-5) edge (m-2-6);
                \end{tikzpicture}
            \end{center}
        }
...
\begin{remark}[Tensor product and internal Hom adjunction in \( \C \)]
    ...
    \begin{center}
        \begin{tikzpicture}
            ...
            \draw[math]
                ...
                (m-2-1) edge (m-2-2)
                (m-2-2) edge node {d_{-1}^C} (m-2-3)
                (m-2-3) edge node {d_0^C} (m-2-4)
                (m-2-4) edge (m-2-5);
        \end{tikzpicture}
    \end{center}
    ...
    \begin{center}
        \begin{tikzpicture}
            ...
            \draw[math]
                (m-1-1) edge (m-1-2)
                (m-1-2) edge node {d_{-1}^A} (m-1-3)
                    edge node {?_{-1}} (m-2-2)
                (m-1-3) edge node {d_0^A} (m-1-4)
                    edge node {?_0} (m-2-3)
                (m-1-4) edge (m-1-5)
                    edge node {?_1} (m-2-4)

                (m-2-1) edge (m-2-2)
                (m-2-2) edge node {d_{-1}^{\class*{B, C}}} (m-2-3)
                (m-2-3) edge node {d_0^{\class*{B, C}}} (m-2-4)
                (m-2-4) edge (m-2-5);
        \end{tikzpicture}
    \end{center}
...
The following is a proof that the above construction is correct.
\begin{proof}[Proof that the construction in \autoref{rem:tensor_prod_internal_hom_adjoint} is the adjoint]\phantom{hei}
    ...
    \begin{enumerate}
        \item {
            ...
            \begin{center}
                \begin{tikzpicture}
                    ...
                    \draw[math]
                        (m-1-1) edge node {d_i^A} (m-1-2)
                            edge node {\phi_{f, i}} (m-2-1)
                        (m-1-2) edge node {\phi_{f, i + 1}} (m-2-2)

                        (m-2-1) edge node {d_i^{\class*{B, C}}} (m-2-2);
                \end{tikzpicture}
            \end{center}
            commutes.
        ...
        \item {
            ...
            % TODO: Forstår ikkje Marius sin skrift på slutten av tlb. 
            \begin{align*}
...
\begin{proof}[Sketch of proof]
    ...
    \begin{enumerate}
        ...
        \item {
            ...
            By uniqueness of a three-tensor version of \autoref{lem:map_out_of_tensor_unique}, it can be shown that the above diagram commutes.
            
            Let the composition of the vertical morphisms be the associativity chain morphism.
        }
        \item[5 \& 6.] {
            ...
            \begin{center}
                ...
                \end{tikzpicture}
            \end{center}
            and the chain complex \( A \otimes I \) is similar.
    ...
    % TODO: Greit å referera til diagram i Borceux?
    \begin{enumerate}
        \item {
