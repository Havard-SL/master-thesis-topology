...
\begin{proof}
    ...
    \begin{diagramlabel}[\label{tikz:differential_of_tensor_product_of_chain_complexes_over_Mod(R)}]
        ...
        \end{tikzpicture}
    \end{diagramlabel}
    where \( \iota_{i, j} \) is as defined in \autoref{not:coprod}, and where the \( g_{i, j} \)'s are \( R \)-bilinenar.
    ...
    \]
    which is exactly equal to \( f \), and \( f \) is therefore uniquely determined by the \( g_{i, j} \)'s.
\end{proof}
...
\begin{remark}
    The definition of the differentials of the internal Hom are well-defined because the only thing that needs to be checked is if the \( d_n \)'s are differentials, which is easy to prove.
...
\begin{remark}[Tensor product and internal Hom adjunction in \( \C \)]
    ...
    1) We want to show that \( \phi_f \) is a chain morphism.
    ...
    2) We want to show that the assignment \( f \mapsto \phi_f \) is an isomorphism of groups.
    ...
    3) We want to show there is a natural isomorphism
    \[
        \C(?_1 \otimes ?_2, ?_3) \cong \C(?_1, \class*{?_2, ?_3}).
    \]
    ...
    TODO: Fullfør argumentet, korleis gir dette likheiten, eg trur det er noko openbart eg ikkje skjønar. Følg opp med å laga ein "sketch" av eit bevis for lemmaet under.
\end{remark}
...
% TODO prove/CITE?
We will omit the proof of the above lemma.
