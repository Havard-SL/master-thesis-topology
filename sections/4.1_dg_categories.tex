...
Any morphism on the form \( f: (A \otimes B)_n \to C \) always has some \( R \)-bilinear \( g_{i, j}: A_i \times B_j \to C \) that uniquely define \( f \). In a way, the \( g_{i, j} \) ``represent'' \( f \).
