...
\begin{remark} \label{rem:F_properties}
    ...
    Where
    \[
        F(f)_{S_i}^{S_j} := p_{S^{n_B} \oplus M^{m_B}}^{S_j} \circ F(f) \circ i_{S_i}^{S^{n_A} \oplus M^{m_A}}
    \]
    and
    \[
        \iota_{S_i}^{S^j} := p_{S^j \oplus M^k}^{S_j} \circ i_{S_i}^{S^j \oplus M^k}.
    \]
    However, one has that \( \iota_{S_i}^{S_j} \) is zero unless \( i = j \). This makes most of the entries in \( A \), except for certain ``blocks'' on the diagonal, equal to \( 0 \).
