...
\begin{proof}[Proof of Massey products = Toda brackets]
    ...
    Let \( \phi: F^{-1} F \to \Id_{\Tc} \) be the natural isomorphism from the definition of Massey products on an algebraic triangulated category. Let \( \eta: F \Sigma \to \Sigma F \) be the natural isomorphism from the fact that \( F \) is a triangulated equivalence. Let \( \mu: F^{-1} \Sigma \to \Sigma F^{-1} \) be the natural isomorphism arising from the fact that \( F^{-1} \) is a triangulated equivalence by \autoref{def:tri_functor}.

    % TODO: Kvifor er dette naturleg? Ganske sikker, treng resultat i forkant?
    Assume that on \( \im(F) \), \( \mu \) is defined as follows: Every object \( A' \in \im(F) \) can be written on the form \( F A \) for some \( A \in \Tc \). Therefore, the following composition defines a valid natural isomorphism,
    \begin{align*}
        \mu: &F^{-1} \Sigma F \to \Sigma F^{-1} F \\
        &= \set{ (\Sigma \phi^{-1}_A) \circ \phi_{\Sigma A} \circ (F^{-1} \eta^{-1}_A)}_{A \in \Tc}.
    \end{align*}

    % Let \( \psi: \Sigma \Sigma^{-1} \to \Id_{\Tc} \) be the natural isomorphism of \( \Sigma \) in \( \Tc \).
    ...
    \begin{center}
        ...
        \begin{tikzpicture}
            \diagram{m}{1cm}{3.5cm} {
                F X_3 \& \Sigma F \Sigma^{-1} Y \\
                F X_3 \& F X_4, \\
            };

            \draw[math]
                (m-1-1) edge node {\eta_{\Sigma^{-1} Y} \circ (F \iota)} (m-1-2)
                    edge[equality] (m-2-1)
                (m-1-2) edge node {(F \beta) \circ \eta^{-1}_{\Sigma^{-1} Y}} (m-2-2)

                (m-2-1) edge node {F f_3} (m-2-2);
        \end{tikzpicture}
    \end{center}
    where
    \[
        \tuple{F (-\Sigma^{-1} \pi), F f_2, (F \beta) \circ \eta^{-1}_{\Sigma^{-1} Y}}
    \]
    is a distinguished triangle by the triangulated functor property of \( F \).

    This yields an element of the Toda bracket,
    \begin{align*}
        (F \beta) \circ \eta^{-1}_{\Sigma^{-1} Y} \circ (\Sigma F &\alpha) \\
        &= (F \beta) \circ \eta^{-1}_{\Sigma^{-1} Y} \circ \eta_{\Sigma^{-1} Y} \circ (F \Sigma \alpha) \circ \eta^{-1}_{X_1} \\
