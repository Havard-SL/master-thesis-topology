        ...
        \begin{tikzpicture}
            ...
            \draw[math]
                (m-1-1) edge node {f} (m-1-2)
                    edge[swap] node {\kappa_A} (m-2-2)
                (m-1-2) edge node {\alpha} (m-1-3)
                    edge[curve={height=-25pt}] node {0} (m-1-4)
                (m-1-3) edge node {\beta} (m-1-4)

                (m-2-2) edge[curve={height=15pt}] node {\rho_A} (m-1-4)
                    edge node {\gamma} (m-1-3);
        \end{tikzpicture}
    \end{center}
...
\begin{remark}
    ...
    By known pushout facts, it follows that since \( \kappa_A \) is a monomorphism, then \( \alpha \) is a monomorphism since \( \ker(\alpha) = \ker(\kappa_A) = 0 \). Likewise it follows that \( \coker(\alpha) \cong \coker(\kappa_A) \cong \Sigma A \), and therefore \( \beta \) is a cokernel morphism for \( \alpha \).
\end{remark}
...
\begin{proof}
    ...
    \begin{enumerate}[label={(\bfseries TR\arabic*)}]
        ...
        \item {
            ...
            Therefore the square to the right of \( \psi \) commutes in \( \Mc \), and not in \( \Mod(R) \), hence why \autoref{eq:stmod_tr4} was said to be \emph{almost} commuting. Therefore
