    ...
    \begin{enumerate}[label={(\bfseries TR\arabic*)}]
        ...
        \item {
            ...
            \begin{center}
                ...
                \end{tikzpicture}
            \end{center}
            by \autoref{lem:stmod_pushout_different_injectives_isomorphic} it follows that there exists some \( \alpha: C \to C_n \) such that \( m \circ \alpha^{-1} = \delta \) as well as \( \alpha^{-1} \circ \gamma_n \circ i = \gamma \).
            ...
            \begin{center}
                \begin{tikzpicture}
                    ...
                    \draw[math]
                        ...
                        (m-1-2) edge node {\alpha^{-1} \circ \psi} (m-2-2)

                        (m-2-1) edge node {\gamma} (m-2-2)
                            edge node {\rho_{C_f}} (m-3-2)
                        (m-2-2) edge node {p} (m-3-2);
                \end{tikzpicture}
            \end{center}
            denotes a standard triangle of \( \phi \), and is therefore distinguished.

            % Before the final step, we need to define two morphisms in order to finish the final step.
            
            % Let \( \Sigma' g \) be defined by the cokernel property in the following diagram
            % \begin{center}
            %     \begin{tikzpicture}
            %         \diagram{m}{1cm}{1cm} {
            %             B \& I_B \& \Sigma B \\
            ...
            %             (m-1-3) edge node {\Sigma' g} (m-2-3)

            %             (m-2-1) edge node {\kappa_{C_f}} (m-2-2)
            %             (m-2-2) edge node {\rho_{C_f}} (m-2-3);
            %     \end{tikzpicture}
            % \end{center}
            % And let \( \epsilon \) be the morphism that is induced by the cokernel property of \( \Sigma C_f \) in the following diagram
            % \begin{center}
            %     \begin{tikzpicture}
            %         \diagram{m}{1cm}{1cm} {
            %             C_f \& I_C{_f} \& \Sigma C_f \\
            %             C_f \& I_{C_f} \& \Sigma C_f, \\
            %         };

            %         \draw[math]
            %             (m-1-1) edge node {\kappa_{C_f}} (m-1-2)
            %                 edge[equal] (m-2-1)
            %             (m-1-2) edge node {\rho_{C_f}} (m-1-3)
            %                 edge node {\tilde{i} \circ i} (m-2-2)
            %             (m-1-3) edge node {\epsilon} (m-2-3)

            %             (m-2-1) edge node {\kappa_{C_f}} (m-2-2)
            %             (m-2-2) edge node {\rho_{C_f}} (m-2-3);
            %     \end{tikzpicture}
            % \end{center}
            % which by \autoref{lem:stmod_sigma_well-defined_additive_endofunctor} implies that \( \epsilon \sim \Id_{C_f} \).
