...
\begin{lemma}
    ...
    In addition, \( \alpha \) has the property that \( \alpha \circ g' = h' \) and \( \alpha \circ \gamma = \gamma' \circ i \), where \( i \) is defined in the beginning of the proof.
\end{lemma}
\begin{proof}
    ...
    \begin{enumerate}[label={(\bfseries TR\arabic*)}]
        ...
        \item {
            ...
            \begin{center}
                ...
                \end{tikzpicture}
            \end{center}
            by \autoref{lem:stmod_pushout_different_injectives_isomorphic} it follows that there exists some \( \alpha: C \to C_n \) such that \( m \circ \alpha^{-1} = \delta \) as well as \( \alpha^{-1} \circ \gamma \circ i = \gamma \).
            
            Consider the following diagram
            \begin{equation}
                \label{diag:stmod_tr4_show_psi_phi_equal_something_else}
                \begin{aligned}
                    \begin{tikzpicture}
                        \diagram{m}{1cm}{1cm} {
                            B \& D \\
            ...
            The leftmost square commutes, and the middle square commutes since \( [\alpha \circ \alpha^{-1}] = [\Id_C] \). It remains to show that the rightmost square commutes.
