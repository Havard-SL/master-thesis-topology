...
\begin{definition}[The functor \( \Omega \)]
    ...
    \begin{itemize}
        ...
        \item {
            ...
            We have that for a morphism \( f: A \to B \), there exists a morphism \( p_f \) from the lifting property of projective modules such that the right square commutes. Note that the choice of \( p_f \) is \emph{not neccesarily unique}.
...
\end{lemma}
\begin{proof}
    There are two things that need to be proven. First, in the construction of \( \Omega'(f) \), we have to choose a morphism \( p_f \) from the projective property. Needs to show that if we choose another projective morphism, that \( \Omega \) still yields the same morphism in \( \Mc \). Second, we need to show that if \( f \sim g \), then \( \Omega'(f) \sim \Omega'(g) \).

    To prove the first part, let \( p_f \) and \( \widetilde{p_f} \) be two different projective morphisms that give the morphisms \( \Omega'(f) \) and \( \widetilde{\Omega'(f)} \), respectively. Then we have the following commutative diagram excluding the dotted arrow
    \begin{center}
        \begin{tikzpicture}
            \diagram{m}{1cm}{2cm} {
                {\Omega A} \& {P_A} \& A \\
    ...
    \]
    there exists a morphism \( \phi \) induced by the kernel property of \( \Omega B \), such that the lower triangle in the diagram commutes. However, since \( \iota_B \) is a monomorphism, we also get that the upper triangle commutes. This implies that the morphism \( \Omega'(f) - \widetilde{\Omega'(f)} \) factors through \( P_A \), a projective module. Therefore
    \[
    ...
    Second, we need to show that if \( f \sim g \), then \( \Omega f \sim \Omega g \). Consider the following commutative diagram excluding the dashed arrow,
    \begin{center}
        \begin{tikzpicture}
            \diagram{m}{1cm}{3cm} {
                \Omega A \& P_A \& A \\
...
\begin{proof}
    ...
    First want to show that \( \Omega \) is functorial. Let \( A, B, C \in \Mc \). Then by the definition of \( \Omega \), we have the following commutative diagram
    \begin{center}
        \begin{tikzpicture}
            \diagram{m}{1cm}{2cm} {
                {\Omega A} \& {P_A} \& A \\
...
\begin{definition}[The functor \( \Sigma \)]
    ...
    \begin{itemize}
        ...
        \item {
            ...
            We have that for a morphism \( f: A \to B \), there exists a morphism \( i_f \) from the universal property of injective objects such that the left square commutes. Please note that this morphism is \emph{not neccesarily} unique.
...
\begin{proof}
    ...
    \]
    and since \( \pi_A \) is an epimorphism, we get that the lower triangle commutes. This implies that \( \phi_2 \circ \phi_1 \sim \Id_A \).
    
    By a similar agument that is omitted for brevity, we can show that \( \phi_1 \circ \phi_2 \sim \Id_{\Sigma\Omega A} \), which means that \( [\phi_1] \) and \( [\phi_2] \) are isomorphisms from \( A \) to \( \Sigma\Omega A \).
    ...
    \begin{center}
        ...
        \end{tikzpicture}
    \end{center}
    where from the cokernel property of \( A \), we get an induced morphism \( \theta \), and from the epimorphism property of \( \pi_A \), it makes the lower triangle commute. And since \( R \) is assumed to be a Frobenius ring, this implies that every injective module over \( R \) is projective and vice versa. This implies
    \[
...
\begin{proof}
    ...
    We can prove that \( \alpha \circ \beta \sim \Id_{C'} \) in a similar way.
    ...
    \begin{enumerate}[label={(\bfseries TR\arabic*)}]
        ...
        \item {
            ...
            \begin{center}
                ...
                \end{tikzpicture}
            \end{center} 
            is (uniquely isomorphic to) a standard triangle of \( g \), and therefore distinguished.
        ...
        \item {
            ...
            \begin{center}
                ...
                \begin{tikzpicture}
                    ...
                    \draw[math]
                        (m-1-1) edge node {f'} (m-1-2)
                            edge[tailed] node {\kappa_{A'}} (m-2-1)
                        (m-1-2) edge node {g'} (m-2-2)
            ...
            \begin{center}
                \begin{tikzpicture}
                    ...
                    \draw[math]
                        ...
                        (m-2-1) edge[tailed] node {\kappa_{A'}} (m-2-2)
                        (m-2-2) edge[two headed] node {\rho_{A'}} (m-2-3);
                \end{tikzpicture}
            \end{center}
        ...
        \item {
            ...
            \begin{center}
                ...
                \end{tikzpicture}
            \end{center}
            where the left ``square'' commutes because the top left and bottom left ``triangles'' of the diagram commute, and \( \Sigma' f \) is given by the cokernel property of \( \Sigma A \).
            ...
            \begin{center}
                ...
                \end{tikzpicture}
            \end{center}
            where the inner square is a pushout and the outer ``square'' commutes by \autoref{eq:stmod_tr4_m_n_f_equal_something_else}.
            ...
            The above morphisms fit into the following commutative diagram, where \( c, d \) are the cokernel morphisms,
            \begin{center}
                \begin{tikzpicture}
                    \diagram{m}{1cm}{1cm} {
                        B \& D \\
