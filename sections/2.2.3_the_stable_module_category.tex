...
But in order to properly define \( \Omega \), a couple prerequisites are needed first.
...
\begin{proof}
    ...
    \begin{center}
        ...
        \end{tikzpicture}
    \end{center}
    where from the cokernel property of \( A \), one gets an induced morphism \( \theta \), and from the epimorphism property, it makes the lower triangle commute, which implies
    \[
        \phi_1^B \circ f \sim \Sigma\Omega(f) \circ \phi_1^A,
    \]
    which means that \( \phi_1 \) is a natural isomorphism from \( \Id_{\StMod(R)} \) to \( \Sigma \Omega \), with inverse \( \phi_2 \).
\end{proof}
